...
circulair = True
radius = 50/2 # Set your desired miroir size #[cm]

if grid ==2: 
    miroir_dimension = radius*2/np.sqrt(1-(1/2)**2)#to take inot consideration shift
else:
    miroir_dimension = radius*2
    
a = 1.6 # sag [cm] 
amplitude = 2E-4 #Gaussian deformation [cm]

def gaussian(x, y, x0, y0, s=1):
    return amplitude * np.exp(-((x - x0)**2 + (y - y0)**2) / (2 * s**2)) + x0**2
...
plt.plot(mirror_array, a*(mirror_array**2)/radius**2)
plt.title("target")
plt.show()
